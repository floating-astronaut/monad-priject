...
contract MonadGateAttestationTest {
    ...
    function _attest(uint256 amount, bytes32 result) internal returns (bytes32 returnedId, Log memory log) {
        vm.recordLogs();
        vm.prank(agent);
    ...
    function testAttestationIdIsRecomputableFromEventAlone() public {
        (bytes32 returnedId, Log memory log) = _attest(5, keccak256("result-1"));
        ...
        require(recomputed == log.topics[1], "id in the log does not match a recomputation from the log");
        require(recomputed == returnedId, "recomputed id does not match the returned id");
    }
    ...
    function testNonceAdvancesAcrossAttestations() public {
        (, Log memory first) = _attest(5, keccak256("result-a"));
        (, Log memory second) = _attest(5, keccak256("result-b"));
        ...
        require(firstNonce == 0, "first attestation should carry nonce 0");
        require(secondNonce == 1, "nonce did not advance");
        require(first.topics[1] != second.topics[1], "two attestations produced the same id");
    }
    ...
    function testDeniedActionDoesNotConsumeNonce() public {
        ...
        (, Log memory log) = _attest(5, keccak256("result-after-denial"));
        (,,, uint256 nonce) = abi.decode(log.data, (bytes32, uint256, bytes32, uint256));
        require(nonce == 0, "a denied action consumed a nonce");
    }
}
